<div class="sidebar_title icon__ukafka">Kafka消息队列 UKafka</div>

- [概览](api/ukafka-api/README.md)
- [索引](api/ukafka-api/index.md)
- **API**
    - [给实例添加节点 - AddUKafkaInstanceNode](api/ukafka-api/add_ukafka_instance_node)
    - [创建实例 - CreateUKafkaInstance](api/ukafka-api/create_ukafka_instance)
    - [删除实例 - DeleteUKafkaInstance](api/ukafka-api/delete_ukafka_instance)
    - [获取 Kafka 消费组信息 - DescribeUKafkaConsumer](api/ukafka-api/describe_ukafka_consumer)
    - [获取整个实例的信息 - DescribeUKafkaInstance](api/ukafka-api/describe_ukafka_instance)
    - [获取节点机型配置 - GetUKafkaNodeType](api/ukafka-api/get_ukafka_node_type)
    - [检查topic名称是否已存在 - IsUKafkaTopicNameExist](api/ukafka-api/is_ukafka_topic_name_exist)
    - [列出 Kafka 消费组 - ListUKafkaConsumers](api/ukafka-api/list_ukafka_consumers)
    - [列举ukafka框架版本 - ListUKafkaFrameworkVersion](api/ukafka-api/list_ukafka_framework_version)
    - [获取实例列表信息 - ListUKafkaInstance](api/ukafka-api/list_ukafka_instance)
    - [获取 kafka 实例 topic  列表信息 - ListUKafkaTopics](api/ukafka-api/list_ukafka_topics)
    - [规格升降级 - ModifyUKafkaInstanceType](api/ukafka-api/modify_ukafka_instance_type)
    - [磁盘扩容 - ResizeUKafkaDisk](api/ukafka-api/resize_ukafka_disk)
