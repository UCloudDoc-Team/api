# 获取社区镜像列表(登录状态) - DescribeUserCommunityImages
...
| 参数名 | 类型 | 描述信息 | 必填 |
|:---|:---|:---|:---|
| **Action**     | string  | 对应的 API 指令名称，当前 API 为 `DescribeUserCommunityImages`                        | **Yes** |
...
| **SortCondition.Field** | string | 排序条件。<br />- Favor：按热度排序，获取热点镜像；<br />- PubTime：按发布时间排序，获取最新社区镜像；<br />- Price 按价格排序；<br />- CreatedCount 按使用量排序；<br />- ImageUseTime 镜像使用时长排序<br />- FavoritesCount 收藏数排序<br />默认："PubTime" |No|
| **SortCondition.ASC** | string | 是否升序排列 |No|
| **IsFree** | boolean | 是否只筛选免费镜像 |No|
| **IsOfficial** | boolean | 是否只筛选官方镜像 |No|
| **FuzzySearch** | string | 模糊搜索，支持 镜像名称、作者名称 |No|
...
| **ImageSetGroup** | array[[*CompshareImageGroup*](#CompshareImageGroup)] | 镜像详情列表 |No|
| **TotalCount** | int | 总数量 |No|
| **AvailableTotalCount** | int | 版本组可用数量 |No|
...
| 字段名 | 类型 | 描述信息 | 必填 |
|:---|:---|:---|:---|
| **GroupId** | string | 版本组ID |No|
| **ImageName** | string | 镜像名称 |No|
| **ImageDesc** | string | 镜像描述 |No|
| **CreatedCount** | int | 引用创建次数 |No|
| **FavoritesCount** | int | 收藏数 |No|
| **RecommendCount** | int | 推荐数 |No|
| **Status** | string | 镜像组状态 Available 可用；UnAvailable 不可用 |No|
| **Data** | array[[*CompShareImage*](#CompShareImage)] | 镜像数据 |No|
| **ImageUseTime** | int | 镜像使用时长 , 单位：小时 |No|
| **IsFavorite** | boolean | 是否被收藏 |No|
...
```
https://api.ucloud.cn/?Action=DescribeUserCommunityImages
&Region=cn-zj
&Zone=cn-zj-01
&ProjectId=JLlEcFeY
&CompShareImageId=AUorWGSw
&Name=dAluvypk
&Author=xgstErUO
&Offset=5
&Limit=lJoKFURR
&Tag=otTGygbS
&SortCondition.Field=RQQeUpVJ
&SortCondition.ASC=etrBEoyk
&TopId=9
&OrgId=8
&IsFree=true
&IsOfficial=false
&FuzzySearch=Szorvoaj
```
      ...
      "Data": [
        {
          "AuthInfo": 8,
          "Author": "JVYpIsPg",
          "CompShareImageId": "wddaZiqo",
          "Container": "hkGgAguE",
          "Cover": "issZkxqD",
          "CreateTime": "GOsnGZoV",
          "CreatedCount": "BWHMYlcH",
          "Description": "DMJRUNIU",
          "FailedReason": "tGoApdKU",
          "FavoritesCount": "HDOMLIiE",
          "ImageOwnerAlias": "zTOTZtzy",
          "ImageType": "bfjFxmNC",
          "IsOfficial": true,
          "Name": "kINGYVeS",
          "Owner": {},
